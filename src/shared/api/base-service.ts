...
export class BaseService<T> {
  ...
  private static isOfflineLike(error: any): boolean {
    return !BaseService.isOnline() || error?.code === 'ERR_NETWORK';
  }
  ...
  public static enqueue(item: OfflineWriteItem): void {
    ...
    try {
      if (USE_BEARER_AUTH && typeof window !== 'undefined' && window.localStorage) {
  ...
  async getAll(params?: Record<string, any>): Promise<T[]> {
    ...
    try {
      ...
      const offlineLike = BaseService.isOfflineLike(e);
