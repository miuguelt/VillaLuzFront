...
// Helper to detect whether an AlertDialogTitle is present in children (recursively)
function containsAlertDialogTitle(node: React.ReactNode): boolean {
  let found = false
  React.Children.forEach(node as React.ReactNode, (child) => {
    if (found) return
    if (!React.isValidElement(child)) return
    const type = (child as React.ReactElement).type as any
    if (type === AlertDialogPrimitive.Title) {
      found = true
      return
    }
    const children = (child as React.ReactElement).props?.children
    if (children) {
      if (containsAlertDialogTitle(children)) {
        found = true
        return
      }
    }
...
>(({ className, children, overlayClassName, ...props }, ref) => {
  const hasTitle = containsAlertDialogTitle(children)
  return (
    <AlertDialogPortal>
